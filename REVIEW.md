...
### Потенциальные улучшения
- ✅ **CSP в [server/index.js](server/index.js)** — уже имеет `'unsafe-inline'` для `style-src`, что позволяет работать inline-стилям React. Фронтенд на отдельном хосте (Railway), поэтому CSP бэкенда его не трогает. Конфигурация корректна.
